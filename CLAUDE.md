...
This repository uses [Black](https://black.readthedocs.io/) for Python formatting, enforced via pre-commit hooks.

```bash
pip install black pre-commit
pre-commit install       # Install hooks into local git repo
black <file_or_dir>      # Format manually
```

All Python code must be Black-formatted before committing.

## Database Migrations (Alembic)

Apps with complex schemas use Alembic for migrations alongside `reflex db`:

```bash
reflex db init           # Initialize Alembic (first time only)
reflex db makemigrations --message "describe change"
reflex db migrate        # Apply pending migrations
```

Examples using Alembic: `form-designer/`, `customer_data_app/`, `chakra_apps/local_auth/`.
